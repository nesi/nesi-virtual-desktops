...
parse_input(){ 
    ...
    # if [ $1 == "exec" ] || [ $1 == "run" ] || [ $1 == "shell" ];then
        
    # fi

    # # If last arg is .sif use that. Else find.
    # lastarg="${@: -1}"
    # thisdir="$(dirname $0)"
    # if [[ $lastarg = *.sif ]]; then
    #     img_path=$(readlink -f $lastarg)
    # else
    #     local_files=( "$(ls $thisdir/*.sif)" )
    #     eval "$0 $* ${local_files[0]}"
...
create_vnc(){   
    ...
    cmd="singularity --debug $@" #$2  ${verbose} shell --nv ${nohome} ${img_path}"
